...
3.将IP地址和该地址出现的次数转换成2个列表，不添加重复的IP，并要确保IP和对应次数，各自的列表里序号相同
4.找出次数列表中的最大值，并根据该元素的序号找出IP列表中对应的元素，打印信息
5.分别在两个列表中删除上一步找出元素
6.将第4步和第5步循环10次，即得出出现频率TOP10的IP地址和次数
'''
...
with open('F:\\access.txt') as file:
    ...
    for i in log_list:
        l_list = i.split()
        ip.append(l_list[0])
    # 将IP地址和出现次数分别添加到新列表ip_list和count_list中
    for i in ip:
        if i not in ip_list:
            # 确保IP和次数对应关系没错
            ip_list.append(i)
    ...
    n = 1
    for i in range(10):
        # 得到count_list列表的最大值（次数），并根据序号找到另一个列表中的IP地址，并打印输出信息
        x = max(count_list)
        y = count_list.index(x)
        z = ip_list[y]
        print "出现频率第%s多的IP是：%s 共出现%d次" %(n,z,x)
        n = n + 1
        # 在对应列表中删除之前找到元素，用于找出频率第二高的IP和次数
        count_list.remove(x)
        ip_list.remove(z)
end = time.clock()
