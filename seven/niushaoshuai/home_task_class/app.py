...
insert_filed=['username','password','role']
...
def afterreg():
    if request.method == 'POST':
        ...
        if not utils.checkout_user_exist(user_data['username']) and user_data['password'] == user_data['password1']:
...
def afterlogin():
    if  request.method == 'POST':
        user_info_dict=dict(request.form)
        user_data={ k:v[0] for k,v in user_info_dict.items() }
        user_id=utils.getid('user',user_data['username']) 
        user_role=utils.getrole('user',user_data['username'])
        if utils.checkout_user_pass(user_data['username'],user_data['password']):
